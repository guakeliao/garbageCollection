...
combine_sub() {
    ...
    local maxCount=$(calculate_CK_MAX $2)
    local combined_all=""
    local tmp1 tmp2
    local TempBlockCookieInterval="$(echo $TempBlockCookie | perl -pe "{s|~|-|; s|_|-|}" | sed 's/\(\d\+\)-\(\d\+\)/{\1..\2}/g')"
    local TempBlockCookieArray=($(eval echo $TempBlockCookieInterval))
        ...
        case $first_param in
            *${name_js[i]}.js | *${name_js[i]}.ts)
                if [[ -f $dir_log/.ShareCode/${name_config[i]}.log ]]; then
                    ...
                    result=$(combine_sub ${var_name[i]} $i)
                    if [[ $result ]]; then
                        export ${env_name[i]}=$result
                    fi
                fi
...
calculate_CK_MAX() {
    local CK_MAX=0
    local index=$1
    for ((CK=0; CK<=100; CK++)); do
      m=${name_js[index]}
      n=${code_length[index]}            ##单条互助规则中互助码平均长度
      a=$((CK-1))                     ##单条互助规则中互助码的数量，全局互助变量中符号 & 的数量
      b=$((CK-2))                     ##单条互助规则中符号 @ 的数量
      c=$((a*n + b))                  ##单条互助规则总字节数
      d=$((c*CK + a))                 ##全局互助变量总字节数
      [[ $d -le $(getconf ARG_MAX) ]] && CK_MAX=$CK || break
    done
    echo $CK_MAX
}
