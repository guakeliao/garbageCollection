...
*/
const $ = new Env('店铺签到_自用');
const notify = $.isNode() ? require('./sendNotify') : '';
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
const dpqdToken = require('./dpqdToken.js');
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data.code==402) {
            vender=''
            console.log(`第`+num+`个店铺ID:`+token+` 签到活动已失效`)
            message +=`第`+num+`个店铺ID:`+token+` 签到活动已失效\n`
          }else{
