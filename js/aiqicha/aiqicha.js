...
let aqcookie = $.isNode() ? process.env.aqcCookies : "";
...
async function dotask(taskList) {
  for (var o of taskList) {
    switch (o.title) {
      ...
      case "CX12009": //浏览互动
        console.log("开始任务：" + oo[o.title]);
        let HomeQuestionres = await get(
          "smart/getHomeQuestionListAjax?page=2&size=10&type=recommend"
        );
        if (HomeQuestionres.status == 0) {
          qdetail =
            HomeQuestionres.data.list[
              Math.floor(Math.random() * HomeQuestionres.data.list.length)
            ];
          nid = qdetail.nid;
      ...
      case "CX12011": //点赞观点
        ...
        if (qCListres.status == 0) {
          ...
          await get(
            `smart/updownAjax?undoType=0&clientType=app&nid=${nid}&parentId=${pid}`
          );
        }
        break;
      default:
